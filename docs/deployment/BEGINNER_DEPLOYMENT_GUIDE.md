...
4. Name the volume:
   ...
   If Railway already created a volume attached to **aelora-ml**, keep that
   existing volume and do not create a second one. Its displayed name is the
   name you must use in the CLI commands later.
...
Find the volume whose output says both:

~~~text
Attached to: aelora-ml
Mount path: /model-artifacts
~~~
...
~~~bat
railway volume files --volume aelora-ml-volume upload .\models\unisolar_capacity_candidate_v3.skops /unisolar_capacity_candidate_v3.skops
railway volume files --volume aelora-ml-volume list /
~~~

If your volume has a different displayed name, replace only
