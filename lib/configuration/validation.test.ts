...
describe("configuration validation", () => {
  ...
  it("validates panel counts, ratings, tilt, and azimuth", () => {
    ...
    const optionalValues = solarArraySchema.parse({ ...valid, temperatureCoefficientPctC: "", installationDate: "" });
    expect(optionalValues.temperatureCoefficientPctC).toBeUndefined();
    expect(optionalValues.installationDate).toBeUndefined();
  });
  ...
  it("validates inverter and battery operating limits", () => {
    ...
    expect(batteryConfigurationSchema.parse({ enabled: "false", usableCapacityWh: 1000, maxChargePowerW: 1000, maxDischargePowerW: 1000, minSocPct: 10, maxSocPct: 90, roundTripEfficiencyPct: 92, reservePct: 20 }).enabled).toBe(false);
  });
});
